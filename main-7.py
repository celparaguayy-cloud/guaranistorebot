...
# ---------- MEMORIA ----------
# Memoria en RAM del bot: sender -> [(rol, texto), ...]  (rápida y confiable)
HISTORIAL = {}

def leer_historial(sender):
    # Lee de la memoria en RAM (no depende de que Airtable encuentre nada).
    return HISTORIAL.get(sender, [])[-20:]

def guardar(sender, rol, mensaje, canal="messenger"):
    # 1) Guarda en la memoria en RAM (para que Fer RECUERDE la charla)
    HISTORIAL.setdefault(sender, []).append((rol, mensaje))
    if len(HISTORIAL[sender]) > 40:
        HISTORIAL[sender] = HISTORIAL[sender][-40:]
    if len(HISTORIAL) > 2000:
        for k in list(HISTORIAL.keys())[:500]:
            HISTORIAL.pop(k, None)
    # 2) Además lo guarda en Airtable (solo para el visor de Charlas del panel)
